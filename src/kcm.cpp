...
kcm::kcm(const set<monomial>& scok, const set<polynomial>& sk, const vector<polynomial>& vP)
	: _vcok(scok.begin(), scok.end())
{
	set<monomial> tsk;
	for (auto P : sk)
	{
	...
	_vk.resize(tsk.size());
	std::copy(tsk.begin(), tsk.end(), _vk.begin());
	...
	for (int i = 0; i < _vcok.size(); ++i)
	{
		_vMR.push_back(_vcok[i].multiplication_number());
	}
	for (int i = 0; i < _vk.size(); ++i)
	{
		_vMC.push_back(_vk[i].multiplication_number());
	}
}
